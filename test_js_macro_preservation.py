...
def test_js_macro_preservation():
    """测试JS宏保留功能"""
    print("=== 测试JS宏保留功能 ===")
    ...
    try:
        ...
        # 保存到文件进行测试
        output_path = "test_js_macro_preservation.xlsm"  # 使用.xlsm扩展名
        with open(output_path, 'wb') as f:
            f.write(excel_data.getvalue())
        print(f"测试文件已保存: {output_path}")
        
        # 验证输出文件
        verify_js_macro_file(output_path, template_path)
...
def verify_js_macro_file(output_path, template_path):
    """验证输出文件是否包含JS宏"""
    print(f"\n=== 验证JS宏文件 ===")
...
if __name__ == "__main__":
    ...
    test_js_macro_preservation()
    
    print("\n测试完成！")
    print("现在导出的Excel文件应该能保留JS宏了！")
